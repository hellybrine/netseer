...
# https://github.com/hellybrine/netseer.git
...
# add site dirs for system and user installed packages (to import bcc with netseer installed via pipx/venv, or dependencies installed via user)
site.addsitedir("/usr/lib/python3/dist-packages")
site.addsitedir(os.path.expandvars("$PYTHON_USER_SITE"))
import psutil

# netseer version and supported platform
...
if sys.flags.optimize > 0:
    print("Warning: netseer does not function properly with the -O (optimize) flag", file=sys.stderr)
...
try:
    if os.getuid() == 0:
        ...
        if sys.executable.startswith("/snap/"):
            home_dir = home_dir.split("/snap/netseer")[0]
except Exception:
    # fallback if there are no regular users on the system, this will likely be /root
    home_dir = os.path.expanduser("~")
BASE_PATH: typing.Final[str] = os.path.join(home_dir, ".config", "netseer")
try:
    ...
    for part in psutil.disk_partitions():
        if part.fstype == "btrfs":
            ...
            if not os.path.exists(os.path.join(BASE_PATH, "config.json")):
                ...
                print("Warning: running netseer on systems with btrfs is not fully supported due to dev number strangeness and non-unique inodes (this is still fine for most use cases)", file=sys.stderr)
            break
    file_path = os.path.join(BASE_PATH, "config.json")
    with open(file_path, "r", encoding="utf-8", errors="surrogateescape") as json_file:
    ...
    def status(self):
        ...
        if pid:
            try:
                ...
                cmdline = ""
            if "netseer" in cmdline:
                print(f"netseer is currently running with pid {pid}.")
            else:
                print("pidfile exists however netseer was not detected.")
        else:
            print("netseer does not appear to be running.")
...
class NotificationManager:
    ...
    def enable_notifications(self):
        ...
        try:
            ...
            self.system_notification = lambda msg: interface.Notify("netseer", 0, "", "netseer", msg, [], [], 2000)
...
def write_snitch(snitch: dict, write_config: bool = False, write_record: bool = True) -> None:
    ...
    try:
        ...
        NotificationManager().toast("netseer write error", file=sys.stderr)
...
def secondary_subprocess_sha_wrapper(snitch: dict, fan_mod_cnt: dict, proc: dict, p_rfuse: ProcessManager, q_vt: multiprocessing.Queue, q_out: multiprocessing.Queue, q_error: multiprocessing.Queue) -> str:
    ...
    if sha256.startswith("!"):
        ...
        if sha256.startswith("!"):
            ...
            # this is meant for appimages that are run as the same user as the one running netseer, since they are not readable as root
    ...
    for proc_pickle in new_processes:
        ...
        for i in range(2):
            ...
            if notification:
                ...
                NotificationManager().toast(f"netseer: {proc_name} {proc_exe}")
...
def primary_subprocess(snitch, snitch_pipes, secondary_pipe, q_error, q_in, _q_out):
    ...
    while True:
        if not parent_process.is_alive():
            q_error.put("netseer has stopped")
...
def secondary_subprocess(snitch, fan_fd, p_rfuse: ProcessManager, p_virustotal: ProcessManager, secondary_pipe, q_primary_in, q_error, _q_in, _q_out):
    ...
    if snitch["Config"]["DB sql log"]:
        ...
        assert cur.fetchone()[0] == 3, f"Incorrect database version of snitch.db for netseer v{VERSION}"
...
def main_process(snitch: dict):
    """coordinates all netseer subprocesses"""
    # init fanotify
    libc = ctypes.CDLL(ctypes.util.find_library("c"))
    ...
    try:
        while True:
            ...
            if not all(p.is_alive() for p in subprocesses):
                ...
                q_error.put(f"netseer subprocess died, attempting restart, terminate by running `netseer stop` ({dead})")
                break
            if any(p.is_zombie() for p in subprocesses):
                zombies = " ".join([p.name for p in subprocesses if p.is_zombie()])
                q_error.put(f"netseer subprocess became a zombie, attempting restart ({zombies})")
                break
            if sum(p.memory() for p in subprocesses) > 4096000000:
                q_error.put("netseer memory usage exceeded 4096 MB, attempting restart")
                break
        ...
        q_error.put("netseer subprocess exception: %s%s on line %s" % (type(e).__name__, str(e.args), sys.exc_info()[2].tb_lineno))
    if sys.argv[1] == "start-no-daemon":
        return 1
    # attempt to restart netseer (terminate by running `netseer stop`)
    time.sleep(5)
    _ = [p.terminate() for p in subprocesses]
...
def ui_loop(stdscr: curses.window, splash: str) -> int:
    ...
    while True:
        ...
        if execute_query:
            ...
            try:
                with open("/run/netseer.pid", "r") as f:
                    run_status = "pid: " + f.read().strip()
            except Exception:
                run_status = "not running"
            print(f"\033]0;netseer v{VERSION} ({run_status})\a", end="", flush=True)
...
def ui_dash():
    ...
    def serve_layout():
        ...
        return html.Div([
            ...
            html.Footer(f"netseer v{VERSION} ({run_status}) (using {file_path})"),
        ])
    try:
...
def start_netseer():
    ...
    netseer comes with ABSOLUTELY NO WARRANTY. This is free software, and you
    ...
    usage:
        netseer dash|view|status|version|help
                    |    |    |      |       |--> this text
                    |    |    |      |--> version info
                    |    |    |--> show pid
                    |    |--> curses tui
        ...
        systemctl enable|disable|start|stop|restart|status netseer
                   |      |       |     |    |       |--> show status with systemd
                   |      |       |_____|____|--> start/stop/restart netseer
                   |______|--> enable/disable autostart on reboot

    * if systemctl isn't working, recreate the service with `netseer systemd`
    * if you don't use systemd, you can use `netseer start|stop|restart` instead
    * if the daemon isn't working, try `netseer start-no-daemon`
    * if dash isn't working, try `netseer start-dash` to see any errors
    ...
    if len(sys.argv) == 2:
        ...
        if sys.executable.startswith("/snap/"):
            if sys.argv[1] in ["start", "stop", "restart", "systemd"]:
                print("Command not supported by netseer snap, use `snap <command> netseer` or `systemctl <command> snap.netseer.daemon`", file=sys.stderr)
                return 2
        elif sys.executable.startswith("/nix/"):
            if sys.argv[1] in ["start", "stop", "restart", "start-no-daemon"]:
                if sys.argv[1] in ["start", "stop", "restart"]:
                    print("WARNING: built in daemon mode is not supported on Nix, use netseer start-no-daemon or systemctl instead", file=sys.stderr)
                if os.getuid() != 0:
                    print("ERROR: netseer requires root privileges to run", file=sys.stderr)
                    return 1
            elif sys.argv[1] == "systemd":
                print("Command not supported on Nix, add `services.netseer.enable = true;` to your Nix configuration", file=sys.stderr)
                return 2
        ...
        if cur.fetchone()[0] != 1:
            ...
            if user_version <= 2:
                assert not os.path.exists("/run/netseer.pid"), "cannot upgrade database while netseer daemon is running"
        ...
        if sys.argv[1] in ["start", "stop", "restart"]:
            if os.path.exists("/usr/lib/systemd/system/netseer.service") or os.path.exists("/etc/systemd/system/netseer.service"):
                print("Found netseer.service but you are not using systemctl")
                if sys.stdin.isatty():
                    confirm = input(f"Did you intend to run `systemctl {sys.argv[1]} netseer` (y/N)? ")
                    if confirm.lower().startswith("y"):
                        subprocess.run(["systemctl", sys.argv[1], "netseer"])
                        return 0
        # init built in daemon control
        class SeerDaemon(Daemon):
            def run(self):
                main()
        daemon = SeerDaemon("/run/netseer.pid")
        # process command line arguments
        if sys.argv[1] == "start":
            print("starting netseer daemon")
            daemon.start()
        elif sys.argv[1] == "stop":
            print("stopping netseer daemon")
            daemon.stop()
        elif sys.argv[1] == "restart":
            print("restarting netseer daemon")
            daemon.restart()
        ...
        # create systemd service file (intended for installing from PyPI or runnning script directly)
            ...
            with open("/usr/lib/systemd/system/netseer.service", "w") as f:
                f.write(systemd_service)
            subprocess.run(["systemctl", "daemon-reload"])
            print("Wrote /usr/lib/systemd/system/netseer.service\nYou can now run netseer using systemctl")
            return 0
        # simple mode (intended for running from systemd or debugging)
        elif sys.argv[1] == "start-no-daemon":
            assert not os.path.exists("/run/netseer.pid"), "pid file already exists"
            def delpid():
                os.remove("/run/netseer.pid")
            atexit.register(delpid)
            if sys.executable.startswith("/nix/"):
                os.makedirs("/run/netseer", exist_ok=True)
            with open("/run/netseer.pid", "w") as f:
                f.write(str(os.getpid()) + "\n")
            print("starting netseer in simple mode")
        ...
        # web gui (launches browser and detaches from terminal on supported platforms (not snap or nix))
            ...
            print("if this fails, try running `netseer start-dash` to see any errors")
